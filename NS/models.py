...
class GumbleGCN(keras.Model):
    def __init__(self, input_dim, output_dim, k , **kwargs):
        ...
        self.mlp = tf.keras.Sequential([
            tf.keras.layers.Dense(params['hidden1'], activation=tf.nn.relu),
            tf.keras.layers.Dropout(0.5),
            tf.keras.layers.Dense(params['hidden2'], activation=tf.nn.relu),
    ...
    def set_fea_adj(self,adj_matrix,shape):
        self.adj_matrix = adj_matrix
        self.shape = shape
    ...
    def call(self, inputs, training=None):
        ...
        y_dense = tf.sparse.to_dense(y) #validate_indices=False
        ...
        h = self.mlp(hidden, training=training)
